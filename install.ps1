...
# ==============================================================================
try {
    $exePath = "$appDir\ssm-manager.exe"
    Write-Host "Setting compatibility for $exePath to run as administrator..." -ForegroundColor Cyan
    Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers" -Name $exePath -Value "~ RUNASADMIN" -ErrorAction Stop
} catch {
